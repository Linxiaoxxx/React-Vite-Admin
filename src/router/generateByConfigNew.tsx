...
function generateRouterByConfigNew() {
  // const result = []
  ...
  return Object.keys(modules).filter(key => !checkIgnore(key)).reduce((result: any[], key) => {
    const pathFilter = key.replace(/\.\.\/views|\.tsx|\/index/g, '').replace('/$', '$').replace(/\$(\w+)/g, '/:$1')
    const pathMatch = key.match(/\/views(.*?)\/index\.tsx/)
    const path = pathFilter || pathMatch![1] || '/'
    console.log('path', path)
    // result.push(createRouter(path))
...
export default generateRouterByConfigNew
